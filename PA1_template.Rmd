...
```{r}
maxStepsDay = cleandatum[(datum$steps == max(cleandatum$steps)),]
```
...
```
####3 Create a new datumset that is equal to the original datumset but with the missing datum filled in.
```{r}
...
####See the README file in the GitHub repository to see an example of what this plot should look like using simulated datum.
```{r}
